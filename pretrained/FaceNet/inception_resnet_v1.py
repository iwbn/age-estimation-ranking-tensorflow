...
def inception_resnet_v1(inputs, is_training=True,
    ...
    with tf.variable_scope(scope, 'InceptionResnetV1', [inputs], reuse=reuse):
        with slim.arg_scope([slim.batch_norm, slim.dropout],
            ...
            inception_scope = tf.get_variable_scope()
        ...
        for v in tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, inception_scope.name):
            if v.name.endswith('weights:0'):
                tf.add_to_collection(tf.GraphKeys.REGULARIZATION_LOSSES, tf.reduce_sum(tf.pow(v, 2)))
